...
export default function AuthForm({
  ...
  return (
    <div className="relative flex h-screen overflow-hidden items-center justify-center px-3 py-4 sm:px-6 lg:px-8 bg-gradient-to-br from-purple-100 via-violet-200 to-white">
      {/* خلفية بلور وتدرج */}
      ...
      {/* الفورم */}
      <div
        className={`relative z-10 w-full max-w-sm sm:max-w-sm md:max-w-md lg:max-w-md space-y-4 sm:space-y-5 bg-white/30 backdrop-blur-xl px-4 py-5 sm:px-6 sm:py-6 rounded-xl shadow-xl ring-1 ring-white/30 transition-all duration-700 ease-out my-auto ${
      ...
      >
        <div className="text-center">
          ...
          <img
            ...
            className="mx-auto h-8 sm:h-10 md:h-12 lg:h-14 w-auto mb-1 sm:mb-2 transition-transform duration-500 ease-in-out hover:scale-105"
          />
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold tracking-tight text-violet-800">
            {title}
          </h2>
        </div>
        ...
        >
          <Form className="space-y-3 sm:space-y-4">
            ...
            <div>
              <label
                htmlFor="username"
                className="block text-xs sm:text-sm font-medium mb-1 text-violet-800"
              >
                Username
              </label>
              <Field
                ...
                className="block w-full rounded-lg border border-violet-200 bg-white/70 px-3 py-2 sm:py-2.5 text-sm sm:text-base text-violet-900 placeholder-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-400"
                placeholder="Enter your username"
              />
              <ErrorMessage
                name="username"
                component="div"
                className="text-rose-500 text-xs sm:text-sm mt-1"
              />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label
                  htmlFor="password"
                  className="block text-xs sm:text-sm font-medium text-violet-800"
                >
                  Password
                </label>
                <a
                  href="#"
                  className="text-xs sm:text-sm font-semibold text-purple-400 hover:text-purple-600 transition"
                >
                  Forgot password?
                </a>
              </div>
            ...
            {/* API Error */}
            {error && (
              <div className="text-rose-500 text-xs sm:text-sm text-center">
                {error}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex w-full justify-center rounded-lg bg-violet-500 px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold text-white shadow hover:bg-purple-400 transition focus:outline-none focus:ring-2 focus:ring-violet-400 disabled:opacity-50"
            >
        ...
        <p className="mt-3 text-center text-xs sm:text-sm text-violet-700">
          Not a member?{" "}
          <a
            href="#"
